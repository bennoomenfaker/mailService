...
@Service
@AllArgsConstructor
public class KafkaListenerInterHospitalService {

    private final JavaMailSender emailSender;
    private final ObjectMapper objectMapper;
    private final TemplateEngine templateEngine;

    private static final String TOPIC_NAME = "equipment-events";

    @KafkaListener(topics = TOPIC_NAME, groupId = "email-service-group")
    public void listenEquipmentEvent(String message) {
        try {
            ...
            String oldHospitalName = eventNode.path("oldHospitalName").asText();
            String newHospitalName = eventNode.path("newHospitalName").asText();
            ...
            context.setVariable("oldHospitalName", oldHospitalName);
            context.setVariable("newHospitalName", newHospitalName);
            context.setVariable("description", description);
            context.setVariable("firstName", firstName);
            context.setVariable("lastName", lastName);
            context.setVariable("equipmentLink", "http://localhost:3000");
            context.setVariable("addEquipmentLink", "http://localhost:3000/manage-equipments/add-new-equipment-to-hospital?serialCode=" + serialCode); // Ajoute serialCode à l'URL

            String htmlContent = templateEngine.process("changementHospital", context);

            // Envoi des emails
            for (String email : emails) {
                try {
                    sendEmailWithHtmlContent(email,
                            "Transfert d'équipement: " + equipmentName,
                            htmlContent);
